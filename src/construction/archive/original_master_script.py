...
# ---------------------------------------------------------------
# 15.4. Constructing Enriched Inventor Move Panel
# ---------------------------------------------------------------

print("\n--- Constructing Enriched Inventor Move Panel (Version 1: Pre/Post Wide Format) ---")

# --- Step 1: Pivot the Base Performance Panel to Wide Format ---
# This DataFrame should already contain pre, post, and change_* columns for performance metrics.
# Key columns: ['inventor_id', 'from_permco', 'to_permco', 'movement_year']
inventor_move_panel_wide = inventor_perf_wide.copy()


# --- Step 2: Merge Pre-Calculated, Move-Level Metrics ---
# Merge Relative Performance (mover_benchmark_df)
inventor_move_panel_wide = inventor_move_panel_wide.merge(
    mover_benchmark_df,
    on=['inventor_id', 'movement_year', 'from_permco', 'to_permco'],
    how='left'
)


# Merge Event-Based Proximity (event_prox_df)
inventor_move_panel_wide = inventor_move_panel_wide.merge(
    event_prox_df,
    on=['inventor_id', 'movement_year'],
    how='left'
)


# --- Step 3: Add M&A Context for From- and To-Firms at the Time of the Move ---
# We can re-use the 'final_event_panel' which has M&A context for every firm-year
# This is simpler than re-calculating with merge_asof

# a) Merge M&A context for the FROM-firm
inventor_move_panel_wide = inventor_move_panel_wide.merge(
    final_event_panel[['permco', 'data_year', 'ma_deal_role', 'years_from_ma_deal', 'ma_failed_merger']],
    left_on=['from_permco', 'movement_year'],
    right_on=['permco', 'data_year'],
    how='left',
    suffixes=('', '_from_firm_context')
)
inventor_move_panel_wide.drop(columns=['permco', 'data_year'], inplace=True)

# b) Merge M&A context for the TO-firm
inventor_move_panel_wide = inventor_move_panel_wide.merge(
    final_event_panel[['permco', 'data_year', 'ma_deal_role', 'years_from_ma_deal', 'ma_failed_merger']],
    left_on=['to_permco', 'movement_year'],
    right_on=['permco', 'data_year'],
    how='left',
    suffixes=('_from_firm_context', '_to_firm_context')
)
inventor_move_panel_wide.drop(columns=['permco', 'data_year'], inplace=True)


# --- Step 4: Final Save ---
output_filename = os.path.join(OUTPUT_PATH, 'inventor_move_panel_wide.pkl')
inventor_move_panel_wide.to_pickle(output_filename)

print(f"\nWide-format inventor move panel created with {len(inventor_move_panel_wide)} observations.")
print(f"Panel saved to: {output_filename}")
print(f"[Inventor move panel · wide] {inventor_move_panel_wide.shape[0]:,} rows × {inventor_move_panel_wide.shape[1]:,} cols | saved: {output_filename}", flush=True)


print("\n--- Constructing Enriched Inventor Move Panel (Annual Long Format) ---")

# --- Step 1: Start with the Annual Performance Panel and Filter Window ---
# Ensure 'years_from_move' is calculated if not present
if 'years_from_move' not in inventor_performance_annual.columns:
    inventor_performance_annual['years_from_move'] = inventor_performance_annual['filing_year'] - inventor_performance_annual['movement_year']

# If 'filing_year' is missing, reconstruct it from movement_year + years_from_move
if 'filing_year' not in inventor_performance_annual.columns:
    inventor_performance_annual['filing_year'] = (
        inventor_performance_annual['movement_year'] + inventor_performance_annual['years_from_move']
    )

# Filter to the desired event window
window = (-5, 5)
base = inventor_performance_annual[
    inventor_performance_annual['years_from_move'].between(*window)
].copy()

# Build a full grid of years_from_move per mover 
id_cols = ['inventor_id', 'movement_year', 'from_permco', 'to_permco']
movers = base[id_cols].drop_duplicates()
years = np.arange(window[0], window[1] + 1)

grid = movers.loc[movers.index.repeat(len(years))].copy()
grid['years_from_move'] = np.tile(years, len(movers))

# Left-join existing rows; add filing_year for added rows
inventor_move_panel_annual = grid.merge(
    base, on=id_cols + ['years_from_move'], how='left'
)
inventor_move_panel_annual['filing_year'] = (
    inventor_move_panel_annual['movement_year'] + inventor_move_panel_annual['years_from_move']
)

# Zero-fill sums/counts; leave means as NaN
sum_like = [
    'patent_id', 'cites', 'xi_real', 'backward_citations', 'self_citations',
    'top1_forward_citations', 'top10_to_2_forward_citations',
    'cited_patent_forward_citations', 'Uncited_patent_forward_citations',
    'same_first_cpc_subclass'
]
for c in (col for col in sum_like if col in inventor_move_panel_annual.columns):
    inventor_move_panel_annual[c] = pd.to_numeric(
        inventor_move_panel_annual[c], errors='coerce'
    ).fillna(0)

# Rename and tidy
inventor_move_panel_annual = (
    inventor_move_panel_annual
      .rename(columns={'patent_id': 'total_patents'})
      .sort_values(id_cols + ['years_from_move'])
      .reset_index(drop=True)
)

# --- Step 2: Merge Move-Level and Annual-Level Metrics ---
# a) Merge move-level context (will be duplicated for each year of the move)
# Relative Performance (mover_benchmark_df)
inventor_move_panel_annual = inventor_move_panel_annual.merge(
    mover_benchmark_df,
    on=['inventor_id', 'movement_year', 'from_permco', 'to_permco'],
    how='left'
)

# b) Merge annual-level rolling similarity (a 1-to-1 merge on year)
inventor_move_panel_annual = inventor_move_panel_annual.merge(
    rolling_sim_df,
    on=['inventor_id', 'movement_year', 'filing_year'],
    how='left'
)


# --- Step 3: Add Annual M&A Context for From- and To-Firms ---
# This attaches the M&A status of the firm IN THE YEAR THE PATENT WAS FILED.

# a) Merge M&A context for the FROM-firm
inventor_move_panel_annual = inventor_move_panel_annual.merge(
    final_event_panel[['permco', 'data_year', 'ma_deal_role', 'years_from_ma_deal', 'ma_failed_merger']],
    left_on=['from_permco', 'filing_year'],
    right_on=['permco', 'data_year'],
    how='left'
).rename(columns={
    'ma_deal_role': 'ma_deal_role_from_firm',
    'years_from_ma_deal': 'years_from_ma_deal_from_firm',
    'ma_failed_merger': 'ma_failed_merger_from_firm'
}).drop(columns=['permco', 'data_year'])

# b) Merge M&A context for the TO-firm
inventor_move_panel_annual = inventor_move_panel_annual.merge(
    final_event_panel[['permco', 'data_year', 'ma_deal_role', 'years_from_ma_deal', 'ma_failed_merger']],
    left_on=['to_permco', 'filing_year'],
    right_on=['permco', 'data_year'],
    how='left'
).rename(columns={
    'ma_deal_role': 'ma_deal_role_to_firm',
    'years_from_ma_deal': 'years_from_ma_deal_to_firm',
    'ma_failed_merger': 'ma_failed_merger_to_firm'
}).drop(columns=['permco', 'data_year'])


# --- Step 4: Final Save ---
output_filename = os.path.join(OUTPUT_PATH, 'final_inventor_move_panel_annual.pkl')
inventor_move_panel_annual.to_pickle(output_filename)

print(f"\nAnnual-format inventor move panel created with {len(inventor_move_panel_annual)} observations.")
print(f"Panel saved to: {output_filename}")
print(f"[Inventor move panel · annual] {inventor_move_panel_annual.shape[0]:,} rows × {inventor_move_panel_annual.shape[1]:,} cols | saved: {output_filename}", flush=True)
